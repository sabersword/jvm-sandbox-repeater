...
//        return RepeaterResult.builder().success(result.isSuccess()).message(result.getMessage()).data(result.getData().getConfigModel()).build();
        ...
        // 白名单列表
        config.setHttpEntrancePatterns(Lists.newArrayList("^/dev/.*$"));
        // java入口方法
        behaviors.add(new Behavior("com.abchina.its.blockchain.tomcat.DevController", "dev"));
        config.setJavaEntranceBehaviors(behaviors);
        List<Behavior> subBehaviors = Lists.newArrayList();
        // java调用插件
        subBehaviors.add(new Behavior("com.abchina.its.blockchain.tomcat.DevService", "devService"));
